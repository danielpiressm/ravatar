...
public class PointCloudDepth : MonoBehaviour
{
    ...
    private void Update()
    {
        if(_head == null)
        {
           _head = Camera.main.transform.parent.GetComponent<HeadCameraController>().headTransform;
    ...
    public void setPoints(byte[] colorBytes, byte[] depthBytes, bool compressed, int sizec, int scale)
    {
        ...
        for (int i = 0; i < renderers.Length; i++)
        {
            ...
            mr.material.SetVector("_HeadPosition", _head.transform.position);
            mr.material.SetFloat("_HeadSize",_headSize );
        }
